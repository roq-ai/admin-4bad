...
import { getClientManagers, deleteClientManagerById } from 'apiSdk/client-managers';
import { ClientManagerInterface } from 'interfaces/client-manager';

type ColumnType = ColumnDef<ClientManagerInterface, unknown>;

interface ClientManagerListPageProps {
...
export function ClientManagerListPage(props: ClientManagerListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getClientManagers({
        ...
        order: params.order,
        searchTermKeys: ['client_portfolio.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ClientManagerInterface>>(
    () => `/client-managers?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteClientManagerById(id);
      await mutate();
    } catch (error) {
  ...
  const handleView = (row: ClientManagerInterface) => {
    if (hasAccess('client_manager', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/client-managers/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'client_portfolio', header: 'Client Portfolio', accessorKey: 'client_portfolio' },
    {
      id: 'start_date',
      header: 'Start Date',
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/client-managers/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('client_manager', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/client-managers/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('client_manager', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="client_manager" />
        </Flex>

        {hasAccess('client_manager', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/client-managers/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
