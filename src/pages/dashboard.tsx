...
import { getClientManagers } from 'apiSdk/client-managers';
...
function DashboardPage() {
  ...
  const clientManagerStat = useSWR<PaginatedInterface<ClientManagerInterface>>(
    () => `/client-managers`,
    () => getClientManagers({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: clientManagerStat?.data?.totalCount,
      isLoading: clientManagerStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
